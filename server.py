...
def run_app(debug: bool = False, bind_all: bool = True):
    if bind_all:
    ...
    if port is None:
       # os.environ["WERKZEUG_RUN_MAIN"] = "true"  # hides starting message
        if not debug:
            ...
            log.disabled = True #was true
            app.logger.disabled = True #was true

        port = _port or get_free_tcp_port()
        # Start in a new thread to not block notebook execution
        # working with debug = False
        t = threading.Thread(
            target=run_app, kwargs={"debug": False, "bind_all": bind_all}
        )
        t.start()
