...
def get_pretrained_model():
    ...
    pt_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "models/trojAImodel.pt")
    )
    #net.load_state_dict(torch.load(pt_path))
    #return net
    #model = torch.load(pt_path, "cpu")
...
def formatted_data_iter():
    ##fails silently with data/test2
    # dataset = torchvision.datasets.CIFAR10(
    #     root="data/test", train=False, download=True, transform=transforms.ToTensor()
    # )
    dataset = CIFAR10(
        root="data/test2", train=False, download=False, transform=transforms.ToTensor()
    )
    dataloader = iter(
...
##from torchvision.datasets.cifar.py
